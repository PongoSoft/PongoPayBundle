...
public function request3DSecureValidation($gatewayReference)
  {
    $this->setPaymentType('checkTransactionDetails');

    $url = $this->api_url . '?merchantId=' . $this->merchant_id;
    $url .= '&paymentType=' . $this->payment_type;

    if(isset($gatewayReference))
    {
      $url .= "&gatewayReference=" . $gatewayReference;
    }
    else
    {
      throw new \Exception("You need to provide a value for the gatewayReference parameter");
    }
